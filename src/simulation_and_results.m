...
%%
rng('shuffle')
Rsym         = 1e6;            % Symbol rate [symbols/s] – adjust as needed
...
%
cyclic_prefix = 1; % Impact of CP, %1 enable CP, %0 disable
...
frameLenSym = 160;                             % Number of symbols for RRC impulse test
cfgWim.NumTimeSamples      = frameLenSym * 40; % Number of time samples for channel synthesis
...
%         end
        ...
        impSym = [1; zeros(frameLenSym-1,1)];
        ...
        h_eq     = h_eq(1:frameLenSym);         % Truncate to known length
